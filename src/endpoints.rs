...
pub async fn mission_post(
    ...
    // If we've sent all items but haven't received an ACK, wait a bit longer
    info!("All waypoints sent, waiting for final ACK");
    if let Ok(MissionMessage::Ack(ack)) = mission_rx.recv_timeout(TIMEOUT) {
        if ack == mavlink::common::MavMissionResult::MAV_MISSION_ACCEPTED {
            info!("Mission upload successful");
            return Ok(HttpResponse::Ok().body("Mission upload successful"));
        } else {
            warn!("Mission upload failed with ACK: {:?}", ack);
            return Ok(
                HttpResponse::InternalServerError().body(
                    format!("Mission upload failed: {:?}", ack)
                )
            );
        }
    } else {
        warn!("Timeout waiting for final MISSION_ACK");
        return Ok(
            HttpResponse::RequestTimeout().body("Timeout waiting for final mission acknowledgement")
        );
    }
}
    */
